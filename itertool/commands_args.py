...
def populate_resample_arguments(subparsers, func):
    parser_resample = subparsers.add_parser('resample', help='Run a post-calibration resampling process as defined in provided script.')
    ...
    parser_resample.add_argument('-r', '--restart', default=None, type=int,
                                 help='Restart resampling at this resampling step (0-indexed) (Default: start at beginning).')
...
def populate_resume_arguments(subparsers, func):
    ...
    parser_resume.add_argument(dest='config_name', default=None,
                               help='Name of configuration python script for custom running of calibration.')
    parser_resume.add_argument('--iteration', default=None, type=int,
                               help='Resume calibration from iteration number (default is last cached state).')
    parser_resume.add_argument('--iter_step', default=None,
                               help="Resume calibration on specified iteration step ['commission', 'analyze', 'plot', 'next_point'].")
