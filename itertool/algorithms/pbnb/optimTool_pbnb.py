...
class OptimToolPBnB(NextPointAlgorithm):
    def __init__(self, params, s_running_file_name, s_problem_type, constrain_sample_fn=lambda s: s,
                 f_delta=par.f_delta, f_alpha=par.f_alpha, i_k_b=par.i_k_b, i_n_branching=par.i_n_branching,
                 i_c=par.i_c, i_replication=par.i_replication, i_stopping_max_k=par.i_stopping_max_k,
        ...
        self.i_dimension_true = len(
            [p for p in self.params if p['Dynamic'] is True])  # delete the false dynamic dimension
        self.dict_n_branching_dim = {p['Name']: 0 for p in params if p['Dynamic'] is True}  # use for branching
        self.f_epsilon = f_elite_worst_sampling_para * pow(max([p['Max'] - p['Min'] for p in self.params]),
                                                           self.i_dimension_true)
        ...
        self.l_subr.append(cSubRegion(self.l_coordinate_lower, self.l_coordinate_upper, self.params))  # <-- SIGMA_1={S}
                ...
                while (self.i_k_c < self.i_k_b or self.i_k == 1) and self.i_k <= self.i_stopping_max_k:  # (3)
                    ...
                    # step 5: Maintain, prune, and branch
                        ...
                        if any(c_subr.s_label == 'M' for c_subr in self.l_subr):
                            fun_plot2D(self.l_subr, self.l_subr[0].l_coordinate_lower,
                                       self.l_subr[0].l_coordinate_upper, self.params, self.i_k,
                                       self.s_running_file_name, iteration)

                        # determine the next step
                        self.b_stopping_branchable = all(
                            i.b_branchable is False for i in (i for i in self.l_subr if i.b_activate is True)) or all(
                            i.s_label != 'C' for i in
                            (i for i in self.l_subr if i.b_activate is True))  # any subregion is branchable
    ...
    def set_results_for_iteration(self, iteration, results):
        ...
        for i in range(0, len(self.l_subr)):
            ...
            self.l_subr[i].pd_sample_record.to_csv(self.s_running_file_name + '/All_region_sampling_record/' + name,
                                                   index=True)
        # fun_plot2D(self.l_subr, self.l_subr[0].l_coordinate_lower, self.l_subr[0].l_coordinate_upper, self.params, str(self.i_k))
        with open(self.s_running_file_name + '/All_region_sampling_record/' + 'l_subr_all_simulations_iteration_' + str(
                self.i_k) + '.dat', "wb") as f:
            pickle.dump(self.l_subr, f)
    ...
    def get_state(self):
        optimtool_state = dict(
            ...
            df_testing_samples=self.df_testing_samples.where(~self.df_testing_samples.isnull(), other=None).to_dict(
                orient='list')
        )
        ...
        for c_subr in self.l_subr:
            ...
            optimtool_state_c_subr = {s_c_subr_name: {
                ...
                'pd_sample_record': c_subr.pd_sample_record.where(~c_subr.pd_sample_record.isnull(),
                                                                  other=None).to_dict(orient='list')}}
    ...
    def set_state(self, state, iteration):
        ...
        for c_subr in state['l_subr']:
            c_subr_set = cSubRegion(state['l_subr'][c_subr]['l_coordinate_lower'],
                                    state['l_subr'][c_subr]['l_coordinate_upper'], self.params)
            ...
            c_subr_set.pd_sample_record = pd.DataFrame.from_dict(state['l_subr'][c_subr]['pd_sample_record'],
                                                                 orient='columns')
