...
def viz_compare( I1, I2 ):
    fig = plt.figure(facecolor='w')
    ax = fig.add_subplot(111, facecolor='#dddddd', axisbelow=True)
    ax.plot(t, I1/1000, 'r', alpha=0.5, lw=2, label='Infected (reference)')
    ax.plot(t, I2/1000, 'b', alpha=0.5, lw=2, label='Infected (calibrated)')
    ax.set_xlabel('Time /days')
    ax.set_ylabel('Number (1000s)')
    ax.set_ylim(0,1.2)
    ax.yaxis.set_tick_params(length=0)
...
def run_compare():
    beta, gamma = 0.2, 0.1
    S,I1,R = run_model( beta, gamma )
    beta, gamma = get_config()
    S,I2,R = run_model( beta, gamma )
    viz_compare( I1, I2 )
