...
a3 = Asset(os.path.join(INPUT_PATH, 'dlls', 'reporter_plugins', 'libcustomreport_TBHIV_ByAge.dll'), relative_path='reporter_plugins')
...
task.set_parameter('Listed_Events',
                   ['TotalPos', 'TBMonitoring', 'Delay', 'Blackout', 'TBTestPreDOTSLow', 'TBTestPreDOTSHigh',
                    ...
                    'BCG_Eligible_HIV', 'Disqualify'] + ['ProviderTestNoR'])    # [TODO] zdu: manually add ['ProviderTestNoR']
...
def map_sample_to_model_input(simulation, sample):
    ...
    tags.update(
        {'TB_Smear_Negative_Infectivity_Multiplier': simulation.task.get_parameter('TB_Smear_Negative_Infectivity_Multiplier'),
...
if calibration_on:
    ...
    calib_manager = CalibManager(name='SA_small_radius_CD4_HIV_death_with_TB_signs_demo',  # <-- Please customize this name
                                 task=task,
    ...
    # builder = ModBuilder.from_combos([ModFn(setRunNumber, g) for g in range(0, 100)],
    #                                  [ModFn(Add_Drugs, resist) for resist in [0.0, 1.0e-1]],
    #                                  [ModFn(map_sample_to_model_input, ss) for ss in [subsample]]
    #                                  )
    ...
    fs1 = [ModFn(partial(EMODTask.set_parameter_sweep_callback, param="Run_Number", value=i)) for i in range(0, 3)]   # 100
...
if __name__ == "__main__":
    ...
    # [TODO]: zdu: manually testing missing events...
    # task.config["Custom_Individual_Events"].extend(["Below200", "Below350", "Below500",  "Above500"])
    # task.set_parameter("Enable_Abort_Zero_Infectivity", 0)

    # # Base collection for the input files
    # common_assets_1 = AssetCollection.from_id('125a2f88-7152-e911-a2c0-c4346bcb1554', as_copy=True)
    # task.common_assets.add_assets(common_assets_1)
    #
    # # Base collection for the DLLs
    # common_assets_2 = AssetCollection.from_id('096222f4-20e5-e811-a2bd-c4346bcb1555', as_copy=True)
    # task.common_assets.add_assets(common_assets_2)
    ...
    if calibration_on:
        ...
        exp_name = 'tb_sa_demo 4'   # [TODO] zdu: test with simple name
        ...
        #     # print(sim.id)
        #     # print(sim.task.config)
        #     print(sim.task.campaign.json)
        #     camp_list.append(sim.task.campaign.json)
        #
        # camp_list_len = [len(a) for a in camp_list]
        # print(camp_list_len)
