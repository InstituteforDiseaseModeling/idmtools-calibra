...
#
# platform = Platform('SLURM2')  # switch to BELEGOST with platform = Platform('BELEGOST')
...
#Sharon comment: can not do following function
...
def update_more_config(task):
    ...
    task.set_parameter('Demographics_Filenames', ["Trial_Demog_SouthAfrica_3.json", 'Base_Overlay_SouthAfrica_ReVacc.json'])
    ...
    task.set_parameter('Listed_Events',
                       ['TotalPos', 'TBMonitoring', 'Delay', 'Blackout', 'TBTestPreDOTSLow', 'TBTestPreDOTSHigh',
                        ...
                        'CRPPosHIVNeg',
                        'LAMPosHIVPos', 'LAMPosHIVNeg', 'TLAM', 'B100', 'None', 'TruePos', 'TruePosHIV', 'B200',
                        'Bmiddle',
    ...
    add_simple_health_seeking(task, ['Below350'], 'Seek350', seek_350_500, start_day=art_start + 3.0 * 365.0,
                              duration=-1)
    add_simple_health_seeking(task, ['Below500'], 'Seek500', seek_350_500, start_day=art_start + 6.0 * 365.0,
                              duration=-1)
...
def general_sim(erad_path, ep4_scripts):
    ...
    task = EMODTask.from_default2(
            ...
            demog_builder=None,  # here we already loaded demo files to Assets and add filename to config's Demographics_Filenames
    ...
    builder.count = len(fs1) * len(fs2) * len(fs3) * len(fs5) * len(fs6)
    ...
    ts.tags.update({'TB_Smear_Negative_Infectivity_Multiplier':
                    ...
                    'Base_Population_Scale_Factor': task.config.parameters.Base_Population_Scale_Factor})
