...
class TB_EMODTask(EMODTask):
    ...
    def set_command_line(self) -> NoReturn:
        ...
        if self.use_embedded_python:
            self.command = CommandLine(f"Assets/{self.executable_name}", "--config",  f"{config.json}",  # noqa
            ...
            self.command = CommandLine(f"Assets/{self.executable_name}", "--config",  "config.json",  # noqa
                                       f"--input-path",  f"{input_path}", f"--dll-path",  "./Assets")  # noqa
